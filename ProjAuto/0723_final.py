...
def create_requirements_traceability_matrix_document(project_info, requirements_df, testcases_df, doc_history_df):
    document = Document()
    set_korean_font(document) # 한글 폰트 설정 적용
    ...
    if not requirements_df.empty and not testcases_df.empty:
        # 요구사항과 테스트케이스를 병합하여 추적표 생성
        # requirements_df의 '테스트케이스ID_목록'을 풀어헤치고 testcases_df와 조인
        # 만약 요구사항에 테스트케이스가 없으면, 요구사항만 표시
        
        # '테스트케이스ID_목록'이 비어있지 않은 경우에만 explode
        temp_reqs_df = requirements_df.copy()
        temp_reqs_df['테스트케이스ID_목록'] = temp_reqs_df['테스트케이스ID_목록'].apply(lambda x: x if isinstance(x, list) else [])
        
        exploded_reqs_df = temp_reqs_df.explode('테스트케이스ID_목록')

        # 테스트케이스가 없는 요구사항도 포함하기 위해 outer merge 사용
        merged_df = pd.merge(
            exploded_reqs_df,
            testcases_df,
            left_on='테스트케이스ID_목록',
        ...
        req_id_col = '요구사항ID_req' if '요구사항ID_req' in merged_df.columns else '요구사항ID'
        req_name_col = '요구사항 명_req' if '요구사항 명_req' in merged_df.columns else '요구사항 명'
        req_desc_col = '요구사항 설명_req' if '요구사항 설명_req' in merged_df.columns else '요구사항 설명'

        # 필요한 컬럼만 선택하고 순서 조정
        trace_cols_mapping = {
            req_id_col: '요구사항ID',
            req_name_col: '요구사항 명',
            req_desc_col: '요구사항 설명',
            '테스트케이스ID': '테스트케이스ID',
            '테스트케이스 명': '테스트케이스 명',
            '테스트케이스 설명': '테스트케이스 설명',
            '테스트 방법': '테스트 방법'
        }
        valid_cols = [col for col in trace_cols_mapping.keys() if col in merged_df.columns]
        final_trace_df = merged_df[valid_cols].rename(columns=trace_cols_mapping)

        # NaN 값을 빈 문자열로 대체하여 보기 좋게 만듦
        final_trace_df = final_trace_df.fillna('')

        table = document.add_table(rows=1, cols=len(final_trace_df.columns))
        table.style = 'Table Grid'
        hdr_cells = table.rows[0].cells
        for i, col_name in enumerate(final_trace_df.columns):
            hdr_cells[i].text = col_name
            for paragraph in hdr_cells[i].paragraphs: # 헤더 볼드체
                for run in paragraph.runs:
                    run.bold = True
        
        for index, row in final_trace_df.iterrows():
            row_cells = table.add_row().cells
            for i, col_name in enumerate(final_trace_df.columns):
                row_cells[i].text = str(row.get(col_name, ''))
    elif not requirements_df.empty:
        document.add_paragraph("등록된 요구사항은 있으나, 연결된 테스트케이스가 없어 추적표의 테스트케이스 섹션이 비어있습니다.")
        # 요구사항만 있는 경우를 위한 테이블 (테스트케이스 컬럼은 비워둠)
        display_cols = ['요구사항ID', '요구사항 명', '요구사항 설명', '테스트케이스ID', '테스트케이스 명', '테스트케이스 설명', '테스트 방법']
        table = document.add_table(rows=1, cols=len(display_cols))
        table.style = 'Table Grid'
        hdr_cells = table.rows[0].cells
        for i, col_name in enumerate(display_cols):
        ...
        for index, row in requirements_df.iterrows():
            row_cells = table.add_row().cells
            row_cells[0].text = str(row.get('요구사항ID', ''))
            row_cells[1].text = str(row.get('요구사항 명', ''))
            row_cells[2].text = str(row.get('요구사항 설명', ''))
            row_cells[3].text = '' # 테스트케이스ID
            row_cells[4].text = '' # 테스트케이스 명
            row_cells[5].text = '' # 테스트케이스 설명
            row_cells[6].text = '' # 테스트 방법
    else:
...
def create_gantt_chart_pdf(project_info, schedule_df, show_dependencies=False):
    ...
    if not schedule_df.empty:
        ...
        schedule_df = schedule_df.dropna(subset=['Start Date', 'End Date']).copy()
...
if 'next_id_nf' not in st.session_state:
    st.session_state.next_id_nf = 1
# 요구사항 데이터 프레임 (테스트케이스 ID 목록 포함)
if 'requirements_df' not in st.session_state:
    st.session_state.requirements_df = pd.DataFrame(columns=[
        '요구사항ID', '분류', '요구사항 명', '요구사항 설명', '테스트케이스ID_목록' # JSON array (list of strings)
    ])
# 테스트케이스 데이터 프레임 (소속 요구사항 ID 포함)
if 'testcases_df' not in st.session_state:
    st.session_state.testcases_df = pd.DataFrame(columns=[
        '테스트케이스ID', '소속_요구사항ID', '테스트케이스 명', '테스트케이스 설명', '테스트 방법'
    ])
...
<div style="text-align: center; padding: 2rem 0;">
    <h1 style="color: #1f77b4; font-size: 3rem; margin-bottom: 0.5rem;">🚀 프로젝트 문서 생성기</h1>
    <p style="font-size: 1.2rem; color: #666; margin-bottom: 2rem;">간편하게 프로젝트 설계서를 만들어보세요!</p>
</div>
...
<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); 
            padding: 2rem; border-radius: 20px; margin: 2rem 0; box-shadow: 0 8px 32px rgba(79, 172, 254, 0.3);">
    <h1 style="color: white; margin: 0; display: flex; align-items: center; font-size: 2.5rem;">
        <span style="font-size: 3rem; margin-right: 1rem;">📝</span>
        1. 프로젝트 기본 정보 입력
    </h1>
    <p style="color: #e8f7ff; margin: 1rem 0 0 0; font-size: 1.2rem;">프로젝트의 핵심 정보를 입력하여 문서 생성의 기반을 마련하세요</p>
</div>
""", unsafe_allow_html=True)
...
<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); 
    ...
    <p style="color: #e8f7ff; margin: 1rem 0 0 0; font-size: 1.2rem;">프로젝트에 필요한 요구사항과 테스트케이스를 체계적으로 관리하세요</p>
</div>
""", unsafe_allow_html=True)

st.markdown("### 🔧 2.1. 요구사항 추가 방식 선택")
add_mode = st.radio(
    "요구사항 추가 방식 선택",
...
if add_mode == "새로운 요구사항 추가":
    ...
    with st.form("new_req_form", clear_on_submit=True):
        ...
        st.markdown("#### 🧪 테스트 관련 정보 (선택 사항 - 첫 번째 테스트 케이스)")
    ...
    if submitted:
        if req_name and req_description:
            new_req_id = generate_requirement_id(req_category)
            
            # 요구사항 데이터 프레임에 추가
            new_req_row = pd.DataFrame([{
                '요구사항ID': new_req_id,
                '분류': req_category,
                '요구사항 명': req_name,
                '요구사항 설명': req_description,
                '테스트케이스ID_목록': [] # 초기에 빈 리스트로 시작
            }])
            st.session_state.requirements_df = pd.concat([st.session_state.requirements_df, new_req_row], ignore_index=True)

            # 테스트케이스 정보가 입력되었다면 테스트케이스 데이터 프레임에 추가
            if tc_name and tc_description and tc_method != "선택 안함":
                generated_tc_id = f"TC-{new_req_id}-001"
                new_tc_row = pd.DataFrame([{
                    '테스트케이스ID': generated_tc_id,
                    '소속_요구사항ID': new_req_id,
                    '테스트케이스 명': tc_name,
                    '테스트케이스 설명': tc_description,
                    '테스트 방법': tc_method
                }])
                st.session_state.testcases_df = pd.concat([st.session_state.testcases_df, new_tc_row], ignore_index=True)
                
                # 요구사항에 테스트케이스 ID 연결
                req_idx = st.session_state.requirements_df[st.session_state.requirements_df['요구사항ID'] == new_req_id].index[0]
                st.session_state.requirements_df.at[req_idx, '테스트케이스ID_목록'].append(generated_tc_id)
                st.success(f"요구사항 '{req_name}' (ID: {new_req_id}) 및 첫 번째 테스트케이스 (ID: {generated_tc_id})이(가) 성공적으로 추가되었습니다.")
            else:
                st.success(f"요구사항 '{req_name}' (ID: {new_req_id})이(가) 성공적으로 추가되었습니다. (테스트케이스는 추가되지 않음)")
        else:
            st.warning("🚨 요구사항 명칭과 상세 설명을 모두 입력해주세요.")
elif add_mode == "기존 요구사항에 테스트케이스 추가":
    st.markdown("### 🔗 2.3. 기존 요구사항에 테스트케이스 추가")
    if not st.session_state.requirements_df.empty:
        # 요구사항 선택 드롭다운 (현재 등록된 요구사항 목록 활용)
        unique_req_options = st.session_state.requirements_df[['요구사항ID', '요구사항 명']].drop_duplicates().copy()
        unique_req_options['Display'] = unique_req_options['요구사항 명'] + " (" + unique_req_options['요구사항ID'] + ")"
        
        # DataFrame이 비어있을 경우 예외 처리
        if unique_req_options.empty:
            st.info("💡 등록된 요구사항이 없어 테스트케이스를 추가할 수 없습니다.")
        else:
            ...
            selected_req_info = st.session_state.requirements_df[st.session_state.requirements_df['요구사항ID'] == selected_req_id].iloc[0]
            ...
            if tc_submitted:
                if tc_name and tc_description and tc_method != "선택 안함":
                    # 해당 요구사항에 연결된 테스트케이스 개수 확인 후 다음 ID 생성
                    existing_tcs_count = st.session_state.testcases_df[
                        st.session_state.testcases_df['소속_요구사항ID'] == selected_req_id
                    ].shape[0]
                    
                    new_tc_suffix = existing_tcs_count + 1
                    generated_tc_id = f"TC-{selected_req_id}-{new_tc_suffix:03d}"

                    # 테스트케이스 데이터 프레임에 추가
                    new_tc_row = pd.DataFrame([{
                        '테스트케이스ID': generated_tc_id,
                        '소속_요구사항ID': selected_req_id,
                        '테스트케이스 명': tc_name,
                        '테스트케이스 설명': tc_description,
                        '테스트 방법': tc_method
                    }])
                    ...
                    # 요구사항 데이터 프레임의 해당 요구사항에 테스트케이스 ID 연결
                    req_idx = st.session_state.requirements_df[st.session_state.requirements_df['요구사항ID'] == selected_req_id].index[0]
                    # 리스트 타입 컬럼에 값 추가 시 .loc/.at 사용
                    current_tc_list = st.session_state.requirements_df.at[req_idx, '테스트케이스ID_목록']
                    if not isinstance(current_tc_list, list): # 로딩 시 문자열로 되어있을 경우 대비
                        current_tc_list = json.loads(current_tc_list) if current_tc_list else []
                    current_tc_list.append(generated_tc_id)
                    st.session_state.requirements_df.at[req_idx, '테스트케이스ID_목록'] = current_tc_list

                    st.success(f"테스트케이스 '{tc_name}' (ID: {generated_tc_id})이(가) 추가되었습니다.")
                else:
                    st.warning("🚨 모든 테스트케이스 정보를 입력해주세요.")
    else:
        st.info("💡 등록된 요구사항이 없습니다. 먼저 새로운 요구사항을 추가해주세요.")

st.markdown("### 📊 2.4. 현재 등록된 요구사항 목록")
st.dataframe(st.session_state.requirements_df, use_container_width=True)

st.markdown("### 📊 2.5. 현재 등록된 테스트케이스 목록")
st.dataframe(st.session_state.testcases_df, use_container_width=True)


if st.button("🔄 요구사항 및 테스트케이스 목록 초기화"):
    st.session_state.requirements_df = pd.DataFrame(columns=[
        '요구사항ID', '분류', '요구사항 명', '요구사항 설명', '테스트케이스ID_목록'
    ])
    st.session_state.testcases_df = pd.DataFrame(columns=[
        '테스트케이스ID', '소속_요구사항ID', '테스트케이스 명', '테스트케이스 설명', '테스트 방법'
    ])
...
<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); 
    ...
    <p style="color: #e8f7ff; margin: 1rem 0 0 0; font-size: 1.2rem;">프로젝트 일정을 체계적으로 관리하고 간트 차트로 시각화하세요</p>
</div>
""", unsafe_allow_html=True)
...
<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); 
    ...
    <p style="color: #e8f7ff; margin: 1rem 0 0 0; font-size: 1.2rem;">문서의 버전과 변경 사항을 체계적으로 추적하고 관리하세요</p>
</div>
""", unsafe_allow_html=True)
st.markdown("### 📝 4.1. 새로운 변경 이력 추가")
...
st.markdown("### 📋 4.2. 현재 등록된 문서 변경 이력")
...
<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); 
    ...
    <p style="color: #e8f7ff; margin: 1rem 0 0 0; font-size: 1.2rem;">입력된 모든 정보를 바탕으로 전문적인 프로젝트 문서들을 생성하고 다운로드하세요</p>
</div>
""", unsafe_allow_html=True)
...
if st.button("📦 모든 문서 압축 다운로드", help="프로젝트 기획서, 결과 보고서, 요구사항 명세서, 요구사항 추적표, 간트 차트(PDF), 일정표(XLS)를 생성하여 하나의 ZIP 파일로 다운로드합니다."):
    ...
    reqs_spec_doc_buffer = create_requirements_document(project_info, st.session_state.requirements_df.copy(), st.session_state.document_history.copy())
    reqs_trace_doc_buffer = create_requirements_traceability_matrix_document(project_info, st.session_state.requirements_df.copy(), st.session_state.testcases_df.copy(), st.session_state.document_history.copy())
...
with col_buttons[2]:
    st.download_button(
        label="📑 요구사항 명세서 다운로드",
        data=create_requirements_document(project_info, st.session_state.requirements_df.copy(), st.session_state.document_history.copy()),
...
with col_buttons_2[0]:
    st.download_button(
        label="🔍 요구사항 추적표 다운로드",
        data=create_requirements_traceability_matrix_document(project_info, st.session_state.requirements_df.copy(), st.session_state.testcases_df.copy(), st.session_state.document_history.copy()),
...
if st.button("💾 현재 데이터 ZIP으로 저장", help="현재 입력된 요구사항/테스트케이스 목록, 일정 데이터 및 문서 이력 데이터를 압축 파일로 저장합니다."):
    if not st.session_state.requirements_df.empty or not st.session_state.testcases_df.empty or not st.session_state.schedule_data.empty or not st.session_state.document_history.empty:
        data_zip_buffer = io.BytesIO()
        with zipfile.ZipFile(data_zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            # 요구사항 데이터 CSV로 저장 (테스트케이스ID_목록은 JSON 문자열로 저장)
            req_csv_buffer = io.StringIO()
            # to_csv로 저장하기 전에 list of strings를 JSON string으로 변환
            st.session_state.requirements_df['테스트케이스ID_목록_JSON'] = st.session_state.requirements_df['테스트케이스ID_목록'].apply(lambda x: json.dumps(x))
            st.session_state.requirements_df.drop(columns=['테스트케이스ID_목록']).rename(columns={'테스트케이스ID_목록_JSON': '테스트케이스ID_목록'}).to_csv(req_csv_buffer, index=False, encoding='utf-8-sig')
            zf.writestr("requirements_data.csv", req_csv_buffer.getvalue())
            
            # 테스트케이스 데이터 CSV로 저장
            tc_csv_buffer = io.StringIO()
            st.session_state.testcases_df.to_csv(tc_csv_buffer, index=False, encoding='utf-8-sig')
            zf.writestr("testcases_data.csv", tc_csv_buffer.getvalue())
...
uploaded_data_zip = st.file_uploader(
    "저장된 데이터 ZIP 파일 업로드 (requirements_data.csv, testcases_data.csv, schedule_data.csv, document_history_data.csv 포함)",
    type=["zip"],
    key="upload_data_zip",
    help="요구사항, 테스트케이스, 일정 및 문서 이력 데이터를 포함하는 ZIP 파일을 업로드하세요."
)
...
if uploaded_data_zip is not None:
    try:
        with zipfile.ZipFile(uploaded_data_zip, 'r') as zf:
            ...
            for name in zf.namelist():
                if name == "requirements_data.csv":
                    with zf.open(name) as file:
                        temp_req_df = pd.read_csv(io.TextIOWrapper(file, 'utf-8-sig'))
                        # JSON 문자열로 저장된 '테스트케이스ID_목록'을 다시 리스트로 변환
                        temp_req_df['테스트케이스ID_목록'] = temp_req_df['테스트케이스ID_목록'].apply(lambda x: json.loads(x) if pd.notna(x) else [])
                        st.session_state.requirements_df = temp_req_df
                        req_found = True
                elif name == "testcases_data.csv":
                    with zf.open(name) as file:
                        st.session_state.testcases_df = pd.read_csv(io.TextIOWrapper(file, 'utf-8-sig'))
                        tc_found = True
            ...
            if req_found or tc_found or schedule_found or history_found:
                st.success("✅ 데이터가 성공적으로 불러와졌습니다.")
                
                # ID 카운터 복원 (가장 큰 ID + 1)
                if not st.session_state.requirements_df.empty:
                    functional_ids = st.session_state.requirements_df[st.session_state.requirements_df['요구사항ID'].str.startswith('RQ-F-', na=False)]['요구사항ID'].str.extract(r'RQ-F-(\d+)').astype(float)
                    nonfunctional_ids = st.session_state.requirements_df[st.session_state.requirements_df['요구사항ID'].str.startswith('RQ-NF-', na=False)]['요구사항ID'].str.extract(r'RQ-NF-(\d+)').astype(float)

                if 'functional_ids' in locals() and not functional_ids.empty and pd.notna(functional_ids.max().squeeze()):
                    next_id_f = int(functional_ids.max().squeeze()) + 1
                else:
                    next_id_f = 1

                if 'nonfunctional_ids' in locals() and not nonfunctional_ids.empty and pd.notna(nonfunctional_ids.max().squeeze()):
